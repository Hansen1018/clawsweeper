...
test("publishing the durable review comment leaves competing comments to lease ownership", () => {
  const source = [
    readFileSync("src/clawsweeper-review-comments-workflow.ts", "utf8"),
    readFileSync("src/clawsweeper-review-comment-leases.ts", "utf8"),
  ...
  assert.doesNotMatch(source, /cleanupSupersededReview/);
  assert.match(
    source.slice(applyStart, applyStart + 2400),
    /DurableReviewPublicationBlockedError[\s\S]*rememberSelfMutationUpdatedAt\(\);[\s\S]*throw error/,
  );
});
