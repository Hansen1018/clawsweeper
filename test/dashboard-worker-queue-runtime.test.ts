...
test("hosted eligibility gates Worker forwarding and queue authority persistence", () => {
  ...
  for (const [route, persistenceMarker] of [
    ...
    assert.ok(block.indexOf("hostedTargetEligibility") < block.indexOf(persistenceMarker), route);
  }
});
...
test("branch and source authority probe before target credentials", async () => {
  ...
  for (const route of ["branch", "source"] as const) {
    for (const admission of ["terminal", "retryable", "public"] as const) {
      ...
      try {
        ...
        assert.equal(reserved.status, route === "branch" ? 202 : 200);

        await harness.queue.alarm();

        assert.equal(targetTokens > 0, admission === "public");
        assert.equal(targetReads, admission === "public" ? 1 : 0);
        assert.equal(probes > 1, admission === "public");
        assert.equal(harness.storage.rawHas(reservationKey), admission === "retryable");
        assert.equal(
          sqlCount(harness.storage, "exact_review_queue_deliveries"),
          admission === "retryable" ? 0 : 1,
        );
        if (admission === "retryable") {
          assert.equal(
            (harness.storage.rawGet(reservationKey) as { attempts: number }).attempts,
            0,
          );
        } else if (admission === "public") {
          const state = (await harness.storage.get("exact-review-queue")) as {
...
test("authority reservations are fenced after visibility I/O", async () => {
  for (const route of ["branch", "source"] as const) {
    ...
    try {
      ...
      await probeStarted;
      await harness.storage.delete(reservationKey);
      releaseProbe();
      await alarm;
